...
namespace DotNetty.Buffers
{
    ...
    sealed class PoolChunk<T> : IPoolChunkMetric
    {
        ...
        PoolSubpage<T>[] NewSubpageArray(int size)
        {
        ...
        int AllocateNode(int d)
        {
            ...
            Contract.Assert(value == d && (id & initial) == 1 << d, string.Format("val = {0}, id & initial = {1}, d = {2}", value, id & initial, d));
        ...
        internal void Free(long handle)
        {
            ...
            int bitmapIdx = (int)((ulong)handle >> 32); // todo: revisit
        ...
        {
            this.InitBufWithSubpage(buf, handle, unchecked((int)((ulong)handle >> (sizeof(int) << 3))), reqCapacity);
        }
        ...
        void SetValue(int id, sbyte val)
        {
        ...
        {
            return memoryMapIdx ^ this.maxSubpageAllocs; // remove highest set bit, to get offset
        }
